...
fn remote_advert_name(speaker: &str) -> String {
    // The clamp keeps renames within budget at the source; re-clamping
    // here guards names persisted before the cap existed (BlueZ
    // *rejects* oversized registrations rather than truncating -
    // field-diagnosed as a box silently not advertising).
    format!("{}{}", ble::ADVERT_PREFIX, ble::clamp_speaker_name(speaker))
}
