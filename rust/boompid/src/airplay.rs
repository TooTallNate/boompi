...
async fn run_once(
    ...
    let (airplay_model, airplay_pin) = {
        let s = app.shared.read().await;
        (
            s.settings.airplay_model.clone(),
            s.settings.airplay_pin.clone(),
        )
    };
    let mut cfg_watch = app.subscribe_cfg();
    cfg_watch.mark_unchanged();
    write_config(&name, &airplay_model, airplay_pin.as_deref())?;
...
fn write_config(name: &str, airplay_model: &str, airplay_pin: Option<&str>) -> anyhow::Result<()> {
    ...
    // shairport's default (generic speaker icon). Model string only,
    // never bare feature bits: bit 26 (ThirdPartySpeaker icon) doubles
    // as Authentication_4, making senders demand MFi auth-setup that
    // shairport cannot answer (connection aborts). Apple model strings
    // need no bits and handshake fine.
    ...
    // Pairing-code mode (patch 0003): senders must enter the code once
    // per device (SRP pair-setup; reconnects verify silently). Bit 51
    // (SystemPairing) makes them prompt - and draws the bookshelf icon
    // for non-Apple models as a bonus (Apple strings keep their icons).
    let pin_line = match airplay_pin {
        Some(pin) => {
            format!("  airplay_pin = \"{pin}\";\n  airplay_features_or = \"0x8000000000000\";\n")
        }
        None => String::new(),
    };
    let conf = format!(
...
general = {{
  name = "{escaped}";
{model_line}{pin_line}  output_backend = "pipe";
