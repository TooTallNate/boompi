...
export default function NowPlaying({artist, track, album, volume, onPlay,onPause, onRewind, onFastForward, onVolumeChange, onPositionChange}: NowPlayingProps) {
	const onVolume = useCallback((event) => {
	...
	return <div className={styles.nowPlaying}>
		...
		<div className={styles.volume}>
			<Volume />
			<input type="range" min="0" max="100" onInput={onVolume} value={volume * 100} />
			<Volume level={3} />
		</div>
	</div>
}
