...
- d9e6ea0: Enable AirPlay 2 playback controls with supported senders and apps by pinning a shairport-sync development revision with upstream remote-control support. Keep classic AirPlay as a compatibility option rather than a requirement for playback controls. Preserve the speaker's PCM format and update progress tracking for the new receiver.
...
- 282a92e: Upgrade the appliance base to Buildroot 2026.08 for newer PipeWire,
  WirePlumber, Mesa, and networking packages, including upstream security
  fixes. Keep the existing Raspberry Pi kernel, boot firmware, BlueZ 5.87,
  and external toolchain pins.
- 31bb568: Use the official Home Assistant logo in the box and remote web sidebars, matching the other icons' color and size.
